# Code from https://github.com/VisualComputingInstitute/diffusion-e2e-ft/blob/main/training/train.py
import argparse
import logging
import math
import os
...
from noise import pyramid_noise_like
from loss import ScaleAndShiftInvariantLoss
from utils import replace_unet_conv_in
...
# @torch.no_grad()
def encode_image(vae: AutoencoderKL, image: torch.Tensor) -> torch.Tensor:
    print("Encode Image - Step 1")
    h = vae.encoder.forward(image)

    if not vae.quant_conv:
        raise ValueError("VAE does not have `quant_conv`")
    print("Encode Image - Step 2")
    moments = vae.quant_conv.forward(h)
    print("Encode Image - Step 3")
...
# @torch.no_grad()
def decode_image(vae: AutoencoderKL, latent: torch.Tensor) -> torch.Tensor:
    if not vae.post_quant_conv:
        raise ValueError("VAE does not have `post_quant_conv`")
    print("Decode Image - Step 1")
    z = vae.post_quant_conv.forward(latent)
    print("Decode Image - Step 2")
    image = vae.decoder.forward(z)
    print("Decode Image - Step 3")
    return image
...
def main():
    ...
    if args.noise_type is None:
        logger.warning(
            "Noise type is `None`. This setting is only meant for checkpoints without image conditioning, such as Stable Diffusion."
        )
    ...
    noise_scheduler = cast(DDPMScheduler, noise_scheduler)
    ...
    if args.noise_type is not None:
        ...
        if unet.config["in_channels"] != 8:
            ...
            logger.info(
                "Unet conv_in layer is replaced for RGB-depth or RGB-normals input"
            )
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            for _ in range(len(models)):
                ...
                load_model = cast(UNet2DConditionModel, load_model)
    ...
    #     train_dataloader_hypersim, train_dataloader_vkitti, split1=9, split2=1
    # )
    dataset_id = "Nfiniteai/product-masks-sample"

    train_pre_transform = transforms.Compose(
        [
            transforms.Resize((2048 // 4, 1536 // 4)),
            transforms.CenterCrop((2048 // 4, 1536 // 4)),
            # transforms.ToTensor(),
        ]
    )
    ...
    def custom_collate(batch):
        ...
        images = train_pre_transform(images)
        mask = train_pre_transform(mask)
    ...
    train_dataset = cast(datasets.Dataset, train_dataset)
    train_dataset.set_format("torch", columns=["image", "mask"])
    train_dataset = train_dataset.filter(lambda x: x["image"].shape == (3, 2048, 1536))
    ...
    alpha_prod = noise_scheduler.alphas_cumprod.to(
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # Validity mask: all pixels are valid (1, H, W)
                val_mask = torch.ones(
                    (
                        batch["image"].shape[0],
                        1,
                ...
                timesteps = torch.ones(
                    (rgb_latents.shape[0],), device=rgb_latents.device
                ) * (noise_scheduler.config["num_train_timesteps"] - 1)  # 999
                ...
                if (args.noise_type is None) or (args.noise_type == "zeros"):
                    ...
                    noisy_latents = pyramid_noise_like(rgb_latents).to(
                        accelerator.device
                    )
                ...
                unet_input = (
                    torch.cat(
                        (
                            rgb_latents.to(accelerator.device),
                ...
                model_pred = unet.forward(
                    unet_input, timesteps, encoder_hidden_states, return_dict=False
                )[0]
                ...
                if val_mask.any():
                    ...
                    current_latent_estimate = (
                        current_latent_estimate / vae.config["scaling_factor"]
                    )
                    current_estimate = decode_image(
                        vae, current_latent_estimate.to(weight_dtype)
                    )
                    ...
                    if args.modality == "depth":
                        ...
                        ground_truth = batch["depth"].to(
                            device=accelerator.device, dtype=weight_dtype
                        )
                    elif args.modality == "normals":
                        norm = (
                            torch.norm(current_estimate, p=2, dim=1, keepdim=True)
                            + 1e-5
                        )
                        current_estimate = current_estimate / norm
                        current_estimate = torch.clamp(current_estimate, -1, 1)
                        ground_truth = batch["mask"].to(
                            device=accelerator.device, dtype=weight_dtype
                        )
                    ...
                    estimation_loss = 0
                    if args.modality == "depth":
                        estimation_loss_ssi = ssi_loss(
                            current_estimate, ground_truth, val_mask
                        )
                        if not torch.isnan(estimation_loss_ssi).any():
                            estimation_loss = estimation_loss + estimation_loss_ssi
                    elif args.modality == "background":
                        print(f"current_estimate: {current_estimate.shape}")
                        print(f"ground_truth: {ground_truth.shape}")
                        print(f"val_mask: {val_mask.shape}")
                        estimation_l2_loss = l2_loss(current_estimate, ground_truth)
                        if not torch.isnan(estimation_l2_loss).any():
                            estimation_loss = estimation_loss + estimation_l2_loss
                    else:
                        raise ValueError(f"Unknown modality {args.modality}")
                ...
                # Save some memory by removing:
                # unet_input, noisy_latents, timesteps, val_mask, rgb_latents
                del unet_input
                del noisy_latents
                del timesteps
                del val_mask
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    logger.info(
                        f"Entered Saving Code at global step {global_step} checkpointing_steps {args.checkpointing_steps}"
                    )
                    if accelerator.is_main_process:
